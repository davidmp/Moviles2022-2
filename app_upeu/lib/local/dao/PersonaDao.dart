...
class PersonaLocate extends BaseRepository{
  ...
  Future<List<PersonaModelo>> getAllPersona() async {
    ...
    List<PersonaModelo> listProduct = result.isNotEmpty ? result.map((c) =>
        PersonaModelo.fromObject(c)).toList() : [];
    return listProduct;
  }

  Future<MsgModelo> insertPersona(PersonaModelo persona) async{
    final db = await database;
    var result = await db.rawInsert('INSERT INTO persona(dni, nombre, apellido_paterno, apellido_materno, telefono, genero, correo) '
        ' VALUES (?,?,?,?,?,?,?)',
        [persona.dni, persona.nombre, persona.apellido_paterno, persona.apellido_materno, persona.telefono, persona.genero, persona.correo]);
  ...
  Future<MsgModelo> updatePersona(PersonaModelo persona) async {
    final db = await database;
    int result = await db.update('persona', persona.toMap(), where: 'id = ${persona.id}');
